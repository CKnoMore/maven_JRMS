...
import static com.cxy.model.User.*;
...
//    } else {
    ...
    if (ValidateKit.isEmail(username)) {
      user = dao.findFirstBy(" `user`.email =? AND `user`.deleted_at is null", username);
    } else if (ValidateKit.isMobile(username)) {
      user = dao.findFirstBy(" `user`.mobile =? AND `user`.deleted_at is null", username);
    } else {
      user = dao.findFirstBy(" `user`.username =? AND `user`.deleted_at is null", username);
    }
    if (user != null) {
...
//    } else {
    User user = dao.findFirstBy(" `user`.username =? AND `user`.deleted_at is null", loginName);
    if (user != null) {
      //遍历角色
